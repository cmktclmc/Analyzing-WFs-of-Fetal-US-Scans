...
def cross_validation_split(conv_df, num_splits):
    ...
    for train_index, val_index in skf.split(X, y):
        train_data = [(image, label) for i in train_index for image in image_list[i][0] for label in [image_list[i][1]]]
        val_data = [(image, label) for i in val_index for image in image_list[i][0] for label in [image_list[i][1]]]
...
def fine_tune_model(net, criterion, optimizer, scheduler, conv_df, num_splits=5, early_stopping_rounds=3, resume=False):
    ...
    """
    checkpoint_dir = "/cs/student/projects1/aisd/2023/ckenney/proximity-to-sp-us-videos/checkpoints_sono_new"
    ...
    for train_data, val_data in cross_validation_split(conv_df, num_splits):
        ...
        plt.grid(True)
        plt.show()
        plt.savefig(f'training_validation_loss_plot_fold_{fold_index}_sononew.png')

    print('Finished Training')

    torch.save(net.state_dict(), '/cs/student/projects1/aisd/2023/ckenney/proximity-to-sp-us-videos/sononet_finetuned_newlabels.pth')
